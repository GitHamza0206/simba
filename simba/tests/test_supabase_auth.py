...
@pytest.mark.skipif(
    not supabase_credentials_available, reason="Supabase credentials not configured"
)
...
class TestAuthServiceMocked:
    ...
    async def test_sign_up(self, mock_supabase):
        ...
        result = await AuthService.sign_up(
            email="test@example.com", password="password123", user_metadata={"role": "user"}
        )
    ...
    async def test_sign_up_error(self, mock_supabase):
        ...
        with pytest.raises(ValueError) as excinfo:
            await AuthService.sign_up(email="existing@example.com", password="password123")
    ...
    async def test_sign_up_exception(self, mock_supabase):
        ...
        # Call the service method and expect an error
        with pytest.raises(ValueError) as excinfo:
            await AuthService.sign_up(email="test@example.com", password="password123")
    ...
    async def test_sign_in(self, mock_supabase):
        ...
        # Call the service method
        result = await AuthService.sign_in(email="test@example.com", password="password123")
    ...
    async def test_sign_in_error(self, mock_supabase):
        ...
        # Call the service method and expect an error
        with pytest.raises(ValueError) as excinfo:
            await AuthService.sign_in(email="wrong@example.com", password="wrong-password")
    ...
    async def test_sign_in_exception(self, mock_supabase):
        ...
        # Call the service method and expect an error
        with pytest.raises(ValueError) as excinfo:
            await AuthService.sign_in(email="test@example.com", password="password123")
